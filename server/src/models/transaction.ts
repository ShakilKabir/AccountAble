...
const transactionSchema = new mongoose.Schema({
  ...
  isPaidOrReceived: { type: Boolean, default: false },
...
transactionSchema.pre("save", async function (next) {
  try {
    ...
    const totalDebits = this.debit_entries.reduce(
      (sum, entry) => sum + entry.amount,
      0
    );
    const totalCredits = this.credit_entries.reduce(
      (sum, entry) => sum + entry.amount,
      0
    );
    ...
    for (let entry of this.debit_entries) {
      ...
      if (!account) {
        throw new Error(
          `Debit account not found for ID: ${entry.account_type}`
        );
      }
      if (
    ...
    for (let entry of this.credit_entries) {
      ...
      if (!account) {
        throw new Error(
          `Credit account not found for ID: ${entry.account_type}`
        );
      }
      if (
