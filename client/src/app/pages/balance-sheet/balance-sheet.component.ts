...
export class BalanceSheetComponent implements OnInit {
  ...
  loadBalanceSheet(): void {
    this.transactionService.getBalanceSheet().subscribe(
      (data) => {
        this.assets = [
          { name: 'Total Cash and Bank', balance: data.totalCash },
          {
            name: 'Total Other Current Assets',
            balance: data.totalOtherCurrentAsset,
          },
        ...
        this.liabilities = [
          {
            ...
            balance: data.totalCurrentLiabilities,
          },
          {
            name: 'Total Long-term Liabilities',
            balance: data.totalLongTermLiabilities,
          },
        ];
        ...
        this.equity = [
          { name: 'Total Owner’s Capital', balance: data.totalOwnerCapital },
          {
            ...
            balance: data.totalRetainedEarnings,
          },
          { name: 'Total Other Equity', balance: data.totalOtherEquity },
        ];

        this.totalAssets = this.assets.reduce(
          (acc, asset) => acc + asset.balance,
          0
        );
        this.totalLiabilities = this.liabilities.reduce(
          (acc, liability) => acc + liability.balance,
          0
        );
        this.totalEquity = this.equity.reduce(
          (acc, equityItem) => acc + equityItem.balance,
          0
        );
      },
      (error) => {
  ...
  loadBalanceSheetByDate(): void {
    this.transactionService.getBalanceSheetByDate(this.reportDate).subscribe(
      (data) => {
        ...
        this.totalEquity =
          data.totalOwnerCapital +
          data.totalRetainedEarnings +
          data.totalOtherEquity;
      },
      (error) => {
